...
static std::array<uint64_t, STATE_WORDS> lab_rotate_words_state(const std::array<uint64_t, STATE_WORDS>& s, size_t r) {
    std::array<uint64_t, STATE_WORDS> t{};
    const size_t shift = r % STATE_WORDS;
    for (size_t i = 0; i < STATE_WORDS; ++i) t[(i + shift) % STATE_WORDS] = s[i];
    return t;
}
...
static void test_permutation_sac(const uint64_t RC[ROUNDS][CAP_WORDS], std::ofstream& summary) {
    ...
    for (int t = 0; t < trials; ++t) {
        ...
        for (size_t w = 0; w < STATE_WORDS; ++w) {
            uint64_t d = a[w] ^ b[w];
            for (size_t bit = 0; bit < 64; ++bit) counts[w * 64 + bit] += static_cast<int>((d >> bit) & 1ULL);
        }
    }
    double avg = 0.0;
    ...
    avg /= static_cast<double>(counts.size());
...
static void test_permutation_bit_balance(const uint64_t RC[ROUNDS][CAP_WORDS], std::ofstream& summary) {
    ...
    for (int t = 0; t < trials; ++t) {
        ...
        lab_permute(s, RC);
        for (size_t w = 0; w < STATE_WORDS; ++w) {
            for (size_t b = 0; b < 64; ++b) counts[w * 64 + b] += static_cast<int>((s[w] >> b) & 1ULL);
        }
    }
    double max_dev = 0.0;
    for (size_t i = 0; i < counts.size(); ++i) {
...
static void test_permutation_byte_frequency(const uint64_t RC[ROUNDS][CAP_WORDS], std::ofstream& summary) {
    ...
    double chisq = 0.0;
    for (size_t i = 0; i < counts.size(); ++i) {
        chisq += (static_cast<double>(counts[i]) - expected) * (static_cast<double>(counts[i]) - expected) / expected;
        csv.row(i, counts[i]);
    }
...
static void test_hash_bit_balance(const uint64_t RC[ROUNDS][CAP_WORDS], std::ofstream& summary) {
    ...
    for (int t = 0; t < trials; ++t) {
        ...
        for (size_t i = 0; i < h.size(); ++i) for (size_t b = 0; b < 8; ++b) counts[i * 8 + b] += static_cast<int>((h[i] >> b) & 1u);
    }
    double max_dev = 0.0;
    for (size_t i = 0; i < counts.size(); ++i) {
        double p = (double)counts[i] / trials;
